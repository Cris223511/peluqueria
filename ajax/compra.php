			...
			case 'guardaryeditar':
				if (empty($idcompra)) {
					...
					if ($numeroExiste) {
						...
						$rspta = $compra->insertar($idusuario, (($idlocal != "") ? $idlocal : $idlocalSession), $idproveedor, $tipo_comprobante, $num_comprobante, $moneda, $impuesto, $total_compra, $vuelto, $comentario_interno, $comentario_externo, $_POST["detalles"], $_POST["cantidad"], $_POST["precio_compra"], $_POST["descuento"], $_POST["metodo_pago"], $_POST["monto"]);
				...
				while ($reg = $rspta->fetch_object()) {
					...
					$data[] = array(
						"0" => '<div style="display: flex; flex-wrap: nowrap; gap: 3px">' .
							...
							'<a data-toggle="modal" href="#myModal10"><button class="btn btn-info" style="margin-right: 3px; width: 35px; height: 35px; color: white !important;" onclick="modalDetalles(' . $reg->idcompra . ', \'' . $reg->usuario . '\', \'' . $reg->num_comprobante . '\', \'' . $reg->proveedor . '\', \'' . $reg->proveedor_tipo_documento . '\', \'' . $reg->proveedor_num_documento . '\', \'' . $reg->proveedor_direccion . '\', \'' . $reg->impuesto . '\', \'' . $reg->total_compra . '\', \'' . $reg->vuelto . '\', \'' . $reg->comentario_interno . '\')"><i class="fa fa-info-circle"></i></button></a>' .
							(($reg->estado == 'Iniciado' || $reg->estado == 'Entregado' || $reg->estado == 'Por entregar' || $reg->estado == 'En transcurso' || $reg->estado == 'Finalizado') ?
								((($_SESSION["cargo"] == 'superadmin' || $_SESSION["cargo"] == 'admin_total') ? (mostrarBoton($reg->cargo, $cargo, $reg->idusuario, '<a data-toggle="modal" href="#myModal11"><button class="btn btn-bcp" style="margin-right: 3px; height: 35px;" onclick="modalEstadoCompra(' . $reg->idcompra . ', \'' . $reg->num_comprobante . '\')"><i class="fa fa-gear"></i></button></a>')) : ("")) .
