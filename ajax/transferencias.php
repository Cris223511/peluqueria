		...
		switch ($_GET["op"]) {
			...
			case 'listarDetalle':
				...
				while ($reg = $rspta->fetch_object()) {
					echo '<tr class="filas"><td></td> <td><a href="../files/articulos/' . $reg->imagen . '" class="galleria-lightbox" style="z-index: 10000 !important;"><img src="../files/articulos/' . $reg->imagen . '" height="50px" width="50px" class="img-fluid"></a></td><td>' . $reg->articulo . '</td><td>' . $reg->local . '</td><td>' . (($reg->categoria != "") ? $reg->categoria : "Sin registrar.") . '</td><td>' . (($reg->marca != "") ? $reg->marca : "Sin registrar.") . '</td><td>' . $reg->codigo_producto . '</td><td>' . (($reg->codigo != "") ? $reg->codigo : "Sin registrar.") . '</td><td>' . $reg->stock . '</td><td>' . $reg->stock_minimo . '</td><td>' . $reg->cantidad . '</td><td>' . "<nav>S/. " . number_format((($reg->medida != "Paquetes") ? ($reg->precio_venta == '' ? "0" : $reg->precio_venta) : ($reg->precio_venta_mayor == '' ? "0" : $reg->precio_venta_mayor)), 2) . "</nav>" . '</td><td>' . $reg->medida . '</td></tr>';
				}
				break;
			...
			case 'listarArticulos':
				...
				while ($reg = $rspta->fetch_object()) {
					...
					$data[] = array(
						"0" => ($reg->stock != '0') ? '<div style="display: flex; justify-content: center;"><button class="btn btn-warning" style="height: 35px;" data-idarticulo="' . $reg->idarticulo . '" onclick="agregarDetalle(' . $reg->idarticulo . ',\'' . $reg->nombre . '\',\'' . $reg->local . '\',\'' . (($reg->categoria != "") ? $reg->categoria : "Sin registrar.") . '\',\'' . (($reg->marca != "") ? $reg->marca : "Sin registrar.") . '\',\'' . $reg->medida . '\',\'' . $reg->stock . '\',\'' . $reg->stock_minimo . '\',\'' . (($reg->medida != "Paquetes") ? ($reg->precio_venta == '' ? "0" : $reg->precio_venta) : ($reg->precio_venta_mayor == '' ? "0" : $reg->precio_venta_mayor)) . '\',\'' . $reg->codigo_producto . '\',\'' . (($reg->codigo != "") ? $reg->codigo : "Sin registrar.") . '\',\'' . $reg->imagen . '\'); disableButton(this);"><span class="fa fa-plus"></span></button></div>' : '',
						...
						"8" => $reg->stock_minimo,
						"9" => (($reg->marca != "") ? $reg->marca : "Sin registrar."),
						...
						"14" => "S/. " . number_format($reg->precio_venta_mayor, 2, '.', ','),
						"15" => "S/. " . number_format($reg->ganancia, 2, '.', ','),
						"16" => "S/. " . number_format($reg->comision, 2, '.', ','),
