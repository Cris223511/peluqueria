				...
				while ($reg = $rspta->fetch_object()) {
					...
					$data[] = array(
						"0" => '<div style="display: flex; flex-wrap: nowrap; gap: 3px">' .
							mostrarBoton($reg->cargo, $cargo, '<button class="btn btn-warning" style="margin-right: 3px; height: 35px;" onclick="mostrar(' . $reg->idmarca . ')"><i class="fa fa-pencil"></i></button>') .
							(($reg->estado == 'activado') ?
							...
							mostrarBoton($reg->cargo, $cargo, '<button class="btn btn-danger" style="height: 35px;" onclick="eliminar(' . $reg->idmarca . ')"><i class="fa fa-trash"></i></button>') .
							'</div>',
						"1" => $reg->titulo,
						"2" => $reg->descripcion,
