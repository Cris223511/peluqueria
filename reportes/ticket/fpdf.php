...
class FPDF
{
	...
	function __construct($orientation = 'P', $unit = 'mm', $size = 'A4')
	{
		...
		if (defined('FPDF_FONTPATH')) {
			...
			$this->fontpath = '';
		// Core fonts
		$this->CoreFonts = array('courier', 'roboto-regular', 'hypermarket', 'helvetica', 'times', 'symbol', 'zapfdingbats');
		// Scale factor
		if ($unit == 'pt')
			$this->k = 1;
		elseif ($unit == 'mm')
	...
	function encabezado2($y, $cliente, $telefono, $tipo_documento, $num_documento)
	{
		...
		$this->SetFont('hypermarket', '', 10);
		$this->SetTextColor(0, 0, 0);
		$this->MultiCell(0, 5, mb_convert_encoding("TELÉFONO: " . ($telefono != "" ? $telefono : "SIN REGISTRAR"), 'ISO-8859-1', 'UTF-8'), 0, 'L', false);
	...
	function cuerpo($y, $totalProductos, $totalUnidades, $textoEnMayusculas)
	{
		...
		if ($totalProductos == 0) {
			$y += 8;
		} else if ($anchoTotalTexto > $anchoMaximo) {
			$lineas = ceil($anchoTotalTexto / $anchoMaximo);
			$y += ($lineas) * 4 - 13;
		} else {
			$y -= 7.5;
	...
	function addCols($tab, $aligns, $y)
	{
		...
		foreach ($tab as $lib => $pos) {
			...
			$align = isset($aligns[$lib]) ? $aligns[$lib] : "L";
			$this->Cell($pos, 1, $lib, 0, 0, $align);
			$colX += $pos;
		}
	}
	...
	function addLine($ligne, $tab)
	{
		...
		foreach ($colonnes as $lib => $pos) {
			...
			$this->MultiCell($longCell, 2.6, $texte, 0, $formText);
