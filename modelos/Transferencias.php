...
class Transferencia
{
			...
			if ($origen != $destino) {
				...
				if ($articulo_datos) {
					...
					$sql_insertar_articulo_destino = "INSERT INTO articulo 
													  (idusuario, idcategoria, idlocal, idmarca, idmedida, codigo, codigo_producto, nombre, stock, stock_minimo, precio_compra, precio_venta, 
	...
	{
		$sql = "SELECT 'origen' AS tabla, l.idlocal AS id, l.titulo, u.nombre AS usuario, local_ruc AS ruc FROM locales l LEFT JOIN usuario u ON l.idusuario = u.idusuario WHERE l.estado='activado' AND l.eliminado='0'
				UNION ALL
				SELECT 'destino' AS tabla, l.idlocal AS id, l.titulo, u.nombre AS usuario, local_ruc AS ruc FROM locales l LEFT JOIN usuario u ON l.idusuario = u.idusuario WHERE l.estado='activado' AND l.eliminado='0'";
	...
	{
		$sql = "SELECT 'origen' AS tabla, l.idlocal AS id, l.titulo, u.nombre AS usuario, local_ruc AS ruc FROM locales l LEFT JOIN usuario u ON l.idusuario = u.idusuario WHERE l.idlocal='$idlocal' AND l.estado='activado' AND l.eliminado='0'
				UNION ALL
				SELECT 'destino' AS tabla, l.idlocal AS id, l.titulo, u.nombre AS usuario, local_ruc AS ruc FROM locales l LEFT JOIN usuario u ON l.idusuario = u.idusuario WHERE l.idlocal='$idlocal' AND l.estado='activado' AND l.eliminado='0'";
